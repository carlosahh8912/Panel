...
function get_g_secret_key(){

	$g = new GoogleAuthenticator();
	$key = $g-> generateSecret();
	return $key;
}

function get_g_qr(string $name, string $seccret){
	$qr_code = GoogleQrUrl::generate($name, $seccret, get_sitename());
	return $qr_code;
}

function verify_g_code($seccret, $code){
	$g = new GoogleAuthenticator();
	if($g->checkCode($seccret, $code)){
		return true;
	}
	return false;
}
...
function get_siteemail() {
  return 'webmaster@gavadesign.com';
}
...
function send_email($from, $to, $subject, $body, $alt = null, $bcc = null, $reply_to = null, $attachments = []) {
    ...
    $mail->Host = 'mail.gavadesign.com';  // Specify main and backup SMTP servers
    $mail->SMTPAuth = true;                               // Enable SMTP authentication
    $mail->Username = 'webmaster@gavadesign.com';                 // SMTP username
    $mail->Password = 'GAVA/web_$';                           // SMTP password
...
function send_email_order($from, $to, $subject, $body, $alt = null, $cc = null, $bcc = null, $reply_to = null, $attachments = []) {
	$mail     = new PHPMailer(true);
	$mail->isSMTP();                                      // Set mailer to use SMTP
    // $mail->Host = 'mail.gavadesign.com';  // Specify main and backup SMTP servers
    // $mail->SMTPAuth = true;                               // Enable SMTP authentication
...
function create_menu($links, $slug_active = 'home') {
	$output = '';
	$output .= '<ul class="nav flex-column pt-3 pt-md-0">';
	foreach ($links as $key => $link) {
		if ($_SESSION['permissions'][$key]['r'] == 1) {
			if ($slug_active === $link['slug']) {
				$output .= 
				sprintf(
					'<li class="nav-item active">
						<a class="nav-link d-flex align-items-center justify-content-between" href="%s">
							<span>
